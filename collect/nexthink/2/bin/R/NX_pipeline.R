...
NX_pipe_connection <- function(FileType = "connection", patternin = "^connection.*.gz$", MoveScannedFileToArchive = TRUE) {
  ...
  for (filein in lstFileToPipe) {
    ...
    print(paste("NX_pipe_connection() : try AnonymizedNXFile for ", filein_file, sep=""))
    res <- try(AnonymizeNxFile(paste(INNXFILES, filein_file, sep="/"), paste(ANONYMIZED, filein_file, sep="/"), FileType = FileType))
    print(paste("NX_pipe_connection() : try AnonymizedNXFile for ", filein_file, " ended with class(res)=", class(res), sep=""))
