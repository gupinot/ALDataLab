    ...
    def execute(): Unit = {
        ...
        this.inputFiles.foreach(filein => {
          ...
          val frames = List("iostat", "sysstat").map((tablename) => {
            val df = sqlContext.sql( s"""select date,server,fixname(name) as name,value,gettags(table) tags from usage where table like '${tablename}%'""")
            val cols = df.select("name").distinct.collect.map(_.getString(0))
          ...
          sqlContext.sql("""
            ...
            from
            (select cast(date/1000 as timestamp) as date,server,get_tag(tags,"mount") as mount, get_tag(tags,"device") as device, ceil(sum(used_mb)) as used_mb, sum(used_percent)/100 as used_percent, sum(avail_mb) as avail_mb, ceil(sum(avail_mb)/(1-sum(used_percent/100))) as total_mb, sum(diskq_count) as diskq_count, sum(disktime_msec) as disktime_msec from
            (select date,server,tags,round(coalesce(used_mb,0)) as used_mb,
            coalesce(disktime_msec,0) as disktime_msec, coalesce(diskq_count,0) as diskq_count, coalesce(used_percent,0) as used_percent, round(coalesce(avail_mb,0)) as avail_mb
            from iostat_pivot) t
            group by date, server, tags) u"""
          ).write.mode(SaveMode.Append).parquet(s"${context.dirout()}/iostat")

          sqlContext.sql("""
            select cast(date/1000 as timestamp) as date,server,tags,sum(cpu_percent) as cpu_percent, sum(memvirtualusage_percent) as memvirtualusage_percent, sum(swapusage_percent) as swapusage_percent, sum(memphysusage_percent) as memphysusage_percent from
            (select date,server,tags,coalesce(cpu_percent,0) as cpu_percent, coalesce(memvirtualusage_percent,0) as memvirtualusage_percent, coalesce(swapusage_percent,0) as swapusage_percent, coalesce(memphysusage_percent,0) as memphysusage_percent
            from sysstat_pivot) t
